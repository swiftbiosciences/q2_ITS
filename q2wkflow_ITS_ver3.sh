...
for R1 in ${inputDir}*_R1*.fastq.gz; do #the input directory
    ...
    trimmed=${WD}/Qobj/${prefix}_swift_trimmed.fastq
...
### Import fastq files containing PE merged and not merged read as single read file to QIIME2 ####
     ...
     qiime tools import \
	     ...
	     --input-path ${WD}/manifest.tsv  \
    	     --output-path ${WD}/Qobj/swift_single.qza \
	     --input-format SingleEndFastqManifestPhred33V2

     qiime demux summarize \
	     --i-data ${WD}/Qobj/swift_single.qza \
             --o-visualization ${WD}/Qobj/swift_single_qual.qzv

     ## Use dada2 to denoise, call ASVs, and remove chimeras 
     qiime dada2 denoise-single \
             --i-demultiplexed-seqs ${WD}/Qobj/swift_single.qza \
             --o-table ${WD}/Qobj/swift_single_dada2_ft \
             --o-representative-sequences ${WD}/Qobj/swift_single_dada2_rep \
             --p-trunc-len 0 \
             --o-denoising-stats ${WD}/Qobj/swift_single_dada2_stats
...
for strand in same reverse-complement; do
  qiime feature-classifier classify-sklearn \
        ...
        --i-reads ${WD}/Qobj/swift_single_dada2_rep.qza \
        --p-confidence 0.7 \
        --o-classification ${WD}/Qobj/swift_single_dada2_rep_taxonomy_${strand} \
	--p-read-orientation $strand 

  qiime tools export \
	     --input-path ${WD}/Qobj/swift_single_dada2_rep_taxonomy_${strand}.qza \
	     --output-path ${WD}
		
  mv ${WD}/taxonomy.tsv ${WD}/taxonomy_${strand}.tsv

done

## Choose the better resolving lineage from two strand classification
${script_dir}/mergeStrand_taxonomy.py \
	${WD}/taxonomy_same.tsv \
	${WD}/taxonomy_reverse-complement.tsv > \
	${WD}/swift_single_dada2_rep_taxonomy.tsv

## import the merged taxonomy back to Qobj
   qiime tools import \
        --type FeatureData[Taxonomy] \
        --input-path ${WD}/swift_single_dada2_rep_taxonomy.tsv \
        --output-path ${WD}/Qobj/swift_single_dada2_rep_taxonomy.qza
 
     ## make barplot
   qiime taxa barplot \
        --i-table ${WD}/Qobj/swift_single_dada2_ft.qza\
        --i-taxonomy ${WD}/Qobj/swift_single_dada2_rep_taxonomy.qza \
        --o-visualization ${WD}/Qobj/swift_single_dada2_barplot \
   ...
   qiime taxa collapse \
        --i-table ${WD}/Qobj/swift_single_dada2_ft.qza \
        --i-taxonomy ${WD}/Qobj/swift_single_dada2_rep_taxonomy.qza \
        --o-collapsed-table ${WD}/Qobj/swift_single_dada2_ft_collapsed.qza \
        --p-level 7
 
     ## Export the feature table
     qiime tools export \
             --input-path ${WD}/Qobj/swift_single_dada2_ft_collapsed.qza \
             --output-path ${WD}/Export
